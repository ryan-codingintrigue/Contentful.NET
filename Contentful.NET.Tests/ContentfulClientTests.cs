...
namespace Contentful.NET.Tests
{
    ...
    public class ContentfulClientTests
    {
        ...
        public async Task TestCanMFB()
        {
            ...
            SearchResult<Entry> results = await client.SearchAsync<Entry>(CancellationToken.None,
                new List<ISearchFilter>
                {
                    new EqualitySearchFilter(BuiltInProperties.ContentType, "frequencyAskedQuestion")
                });
